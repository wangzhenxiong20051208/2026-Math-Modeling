...
def export_result3(price,dates,res):
    ...
    import shutil
    if not OUT_XLSX.exists() and TEMPLATE3.exists():
    ...
    for sheet in ["计划购电量","调整购电量"]:
        ...
        for r,d in enumerate(range(31,365), start=2):
            ...
            if sheet=="计划购电量":
                ...
                cost_pa=float(np.sum(price*res[d]["gP"]+1.5*price*up-0.5*price*down)) if sheet=="调整购电量" else float(np.sum(price*res[d]["gP"]))
                ws.cell(row=r,column=147,value=round(float(cost_pa if sheet=="计划购电量" else float(np.sum(price*res[d]["gP"]+1.5*price*up-0.5*price*down))),6))
            else:
                up=(res[d]["gA"]-res[d]["gP"]).clip(min=0); down=(res[d]["gP"]-res[d]["gA"]).clip(min=0)
