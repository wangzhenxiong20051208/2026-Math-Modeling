...
随支撑材料提交的副本位于 `06_支撑材料/源码/`，当前含 $21$ 个脚本。两份副本
**尚未重新同步**，因此目前既不是文件数相同、也不是逐字节相同，差异分两类：

- $5$ 个脚本未随材料提交（见第六节）。其中 $3$ 个是论文附录 λ 节两张表的
  完整生成链，意味着该附录的数字目前**无法由已提交的代码复现**；
- 另有一批脚本在提交之后又做过修正（问题一大 $M$ 的单位、问题三的锚点口径、
  问题二内核默认值、控制台编码等），提交副本仍是修正前的旧版。

引用 `06_支撑材料/源码/` 前请先与本目录重新同步；在同步完成之前，**本目录是
唯一权威版本**。各脚本的修正内容见第七节。
...
**这三份脚本是论文附录 λ 节唯一的生成链，但目前不在 `06_支撑材料/源码/` 内。**
按"支撑材料应能复现论文全部数字"的要求，应一并补交；否则该附录的数字无法
由已提交的材料复现。
