...
def run_backtest(price, dates, load_kw, pv_kw, fc, alpha=0.8, lam=1.0, rho=0.5, E_tar=6000.0, W=28,
                 ...
                 bill_price=None):
    # price：0:00 计划电价（可预测）；bill_price：结算/紧急用的当天实际电价
    ...
    for d in range(0, maxd+1):
        ...
        # 分时段风险分位：每个10分钟用自己的历史误差，不再全天一个Q
        if len(plan_err_hist)>=7:
            arr=np.vstack(plan_err_hist[-W:])
            Qvec=np.quantile(arr, alpha, axis=0)
        else:
            Qvec=np.zeros(N)
        ...
        if use_adjust:
            ...
            for si,Tk in enumerate(Tks):
                ...
                if len(hist)>=7:
                    arr=np.vstack(hist[-W:])
                    Qadj=np.quantile(arr, alpha, axis=0)
                else:
                    Qadj=Qvec[Tk:]
                ...
                E_at_Tk=E_prev
                # 6/12/18：已实现电价可知，余段仍用 0:00 预测（可用已实现偏差修正），不用未到时刻的实际价
                p_adj = p_plan.copy()
                p_adj[:Tk] = p_bill[:Tk]
