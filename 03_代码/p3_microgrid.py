...
口径（与题面及附件5模板一致）：
  ...
  * 附件3预报：每天 0/6/12/18 时发布未来24个整点功率 (kW)。
    ...
    小时→10分钟采用因果分段线性插值（锚点取发布时刻所在时段**已实现**的实测功率，
    即附件 2 的 36h−1 列），正式区间 MAE 189.81 kW，优于按小时取常值 267.40 kW
    与前移一小时 808.24 kW（见 p3_alignment_scan.py / p3_alignment_scan.json）。
