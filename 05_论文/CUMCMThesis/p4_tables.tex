...
\begin{table}[htbp]
  ...
  \begin{minipage}{.92\textwidth}\footnotesize
    注：两个分支都在\textbf{同一条附件 4 实际价格路径}上独立回放并独立标定，费用一栏不可相加。两者均按\textbf{对应交付时段的实际电价}结算（含初始计划费）。主计费口径取退款：被取消的部分按 $50\%$ 的违约电价计，故 result4-3 全年实际发生下调；result4-2 全天冻结，无任何调整量。
  \end{minipage}
\end{table}
