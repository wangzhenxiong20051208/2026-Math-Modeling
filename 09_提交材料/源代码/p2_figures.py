...
def fig1_forecast(det: pd.DataFrame) -> None:
    ...
    # 图内不写"图 N"：编号由 LaTeX 的 \caption 负责，写死在这里会与真实编号冲突
    fig.suptitle("指定日期的净负荷预测、风险余量与实际净负荷", fontsize=12)
...
def fig2_purchase(det: pd.DataFrame) -> None:
    ...
    fig.suptitle("指定日期的计划购电量、紧急购电量与电价", fontsize=12)
    fig.tight_layout(rect=(0, 0, 1, 0.96))
...
def fig3_storage(det: pd.DataFrame) -> None:
    ...
    fig.suptitle("指定日期的日前参考储电量与实际储电量", fontsize=12)
    fig.tight_layout(rect=(0, 0, 1, 0.96))
...
def fig4_monthly(res: dict) -> None:
    ...
    fig.suptitle("逐月费用分解与策略费用对比", fontsize=12)
...
def fig5_cumulative(res: dict) -> None:
    ...
    fig.suptitle("各策略累计费用与跨日储电量", fontsize=12)
    fig.tight_layout(rect=(0, 0, 1, 0.94))
...
def fig6_calibration(daily: pd.DataFrame) -> None:
    ...
    ax1.set_title("紧急购电量的季节分布与逐日费用构成", fontsize=12)
