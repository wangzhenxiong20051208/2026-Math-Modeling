...
def run_day42(n: int, e0: float, params: RiskParams, book: PriceBook,
    ...
    if n == 0:
        # 冷启动：与问题二、三一致，无历史可用，计划购电为零，储能不越储备线，
        # 缺口全部紧急补购
        z = np.zeros(N)
    ...
    n_hat = l_hat - v_hat
    margin = risk_margin4(eps, n, params.alpha, book.PRICE, weighted=weighted)
    n_risk = n_hat + margin
...
def run_strategy42(cal: CalConfig, book: PriceBook, mode: str,
    ...
    e_at[0] = e
    params = RiskParams(alpha=0.80, rho=1.0, lam=LAMBDA_DEFAULT)
...
def run_strategy43(cal: CalConfig3, book: PriceBook, mode: str,
    ...
    e_at = np.full(N_DAY + 1, np.nan)
    e_at[0] = e
    params = RiskParams(alpha=0.60, rho=0.5, lam=LAMBDA_DEFAULT)
    for n in range(N_DAY):
        if n >= cal.start and (n - cal.start) % cal.every == 0:
...
def run_combo4(S: tuple, cal43: CalConfig3, book: PriceBook,
               LOAD, PV, fp, eps3, fo, weighted: bool = True) -> dict:
...
def run_main_strategies(S: dict, cal42: CalConfig, cal43: CalConfig3,
    ...
    recs43_all, cal43_rows = run_strategy43(
        ...
        S["fo"], weighted=weighted, verbose=True)
...
def main() -> None:
    ...
    cal42 = CalConfig(window=10, every=42) if args.quick else CalConfig()
    cal43 = CalConfig3(window=10, every=42) if args.quick else CalConfig3()
    if args.no_cal:
        # 网格塌缩成一个固定参数，等价于不做标定
