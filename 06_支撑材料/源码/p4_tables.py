...
def tab_branches(res: dict) -> str:
    ...
    lines += [
        ...
        r"  \par\vspace{2pt}",
        r"  \begin{minipage}{.92\textwidth}\footnotesize",
        r"    注：两个分支都在\textbf{同一条附件 4 实际价格路径}上、按各自的规则"
        r"独立回放并独立标定，费用一栏不可相加。两者都按实际电价结算，包括"
        r"初始计划费；主计费口径下不允许下调，故 result4-3 的下调量为零。"
        r"期末储电量用于说明跨日状态差异——只比费用而不比期末储量是不完整的。",
        r"  \end{minipage}",
        r"\end{table}",
        "",
    ]
