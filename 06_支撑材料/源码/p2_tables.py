...
SUP = ROOT / "06_支撑材料"
OUT_TEX = ROOT / "05_论文" / "CUMCMThesis" / "p2_tables.tex"
OUT_TEX_TAIL = ROOT / "05_论文" / "CUMCMThesis" / "p2_tables_appendix.tex"
...
def tab_strategy(res: dict) -> str:
    ...
    for k in keys:
        ...
        lines.append(
            ...
            f" & {es} & {rel} \\\\")
    # 事后理想不是可执行方案，理由有两条，都要说清楚：
    # (1) 它每天从主策略的实际日初储电量出发、却不承担日末留存义务，等于每天白拿
    #     一次已付费的库存电量（累计量与折价见附录 app:p2-lambda 的库存口径检验）；
    # (2) 它制定当天计划时已看到当天实际净负荷，紧急购电费恰为 0。
    # 不要再用「日末储电量停在下限」当理由——那对库存的指控对主策略自身的计划轨迹
    # 同样成立（λ=0 使其日末值 334 天全部为 E_MIN），是无效论证。
    idl = st[keys[3]]
    ebar = idl.get("日末日均储电量_kWh")
    if ebar is not None and np.isfinite(ebar):
        note = (r"    注：“事后理想”用当天实际净负荷替换预测值求解同一日前 MILP。"
                r"它每天从主策略的实际日初储电量出发、却不必承担日末留存义务"
                rf"（日末计划电量日均 ${fmt(ebar, 1)}$~kWh），且制定当日计划时"
                r"已看到当天实际净负荷，故\textbf{不构成可执行方案}，"
                r"仅作乐观下界参照；库存口径的修正见附录。")
    else:
        note = (r"    注：“事后理想”用当天实际净负荷替换预测值求解同一日前 MILP，"
