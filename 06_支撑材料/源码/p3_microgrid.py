...
与问题二的差别只在**信息来源**与**计费口径**两处，物理模型完全一致：
  ...
  * 信息来源：光伏不再由历史外推预测，而是直接使用附件 3 中当日已发布的
    ...
    日内比例修正。
  * 计费口径（框架 3.2 节主口径）：计划费 Σp g⁰ 全额照付，调整费另计
...
def _precomp3(m: int) -> dict:
    ...
    y - a + b = g⁰ 把分段调整费用线性化：
        主口径  → ub[b] = 0，lb[y] = g⁰_t，于是 a = (y-g⁰)_+
        退款口径 → lb[y] = 0，a,b ≥ 0，目标中 b 的系数为 -0.5p
...
def main() -> None:
    ...
    if args.report_only:
        ...
        tasks.append({"tag": CTRL_TAG, "S": tuple(ALL_S), "use_new": False,
                      "convention": "main", "cal": cal_kwargs, "keep": False})
        tasks.append({"tag": REFUND_TAG, "S": tuple(ALL_S), "use_new": True,
                      "convention": "refund", "cal": cal_kwargs, "keep": False})
        if args.quick:
