...
def verify_cache_isolation(verbose: bool = True) -> dict:
    """D5 自检：计划缓存按 (tag, n, α, λ, e0) 隔离——**跑出来**，不是"看代码觉得对"。

    背景：问题四在一条实际价格路径上跑三种价格模式（forecast/fixed/oracle）×
    两种计费口径（main/refund）× 不同调整时点集合 S。同一天、同一 (α, λ, e0)
    的计划会因策略不同而不同。`initial_plan4` 的缓存键 = (tag, n, α, λ, e0)，
    **价格不在键里**——价格模式的区分完全压在 tag 上。一旦 tag 漏项（例如忘了
    把 mode 拼进去），先算的策略会把计划塞给后算的策略，而且**不会报错**，只会
    让结果悄悄错。所以需要一个运行期证据，而不是代码审阅结论。

    做法（完全走生产路径 initial_plan4）：
      ① 正向：取正式区间一天的真实净负荷，用两种必然给出不同计划的价格（实际价
          vs 平价）在同一 (n, α, λ, e0) 上、以**两个不同 tag** 先后调用 → 计划
         必须不同；
      ② 命中：同一 tag 重复调用 → 逐位相同（缓存命中语义正确）；
      ③ 负向对照：**同一 tag、不同价格** → 第二次必然命中第一次的计划（因价格不
         在键里）。这一步证明本检查是灵敏的——如果生产代码真的丢了 tag，行为就会
         退化成这个负向对照的结果；
      ④ 顺序无关：清空缓存、交换两次调用顺序，结果与正向逐位一致。
    """
    LOAD, PV, _ = load_attach2()
    n = REPORT_START
    e0 = float(E_INIT)
    ntilde = (LOAD[n] - PV[n]) * TAU                     # 真实净负荷（kWh/段）
    rp = RiskParams(alpha=0.80, rho=1.0, lam=LAMBDA_DEFAULT,
                    window=28, min_samples=10)
    price_real = load_attach4()[n]
    price_flat = np.full(N, float(price_real.mean()))    # 平价：必然与实际价不同
    tag_fc = "forecast|main|(6, 12, 18)"
    tag_fx = "fixed|main|(6, 12, 18)"

    def plan(tag: str, price: np.ndarray) -> np.ndarray:
        return initial_plan4(n, ntilde, e0, price, rp, tag)[0]

    chk: dict = {}

    _PLAN0_CACHE4.clear()
    g_fc = plan(tag_fc, price_real)
    g_fx = plan(tag_fx, price_flat)
    cross = float(np.abs(g_fc - g_fx).max())
    chk["正向_不同tag计划最大差_kWh"] = cross
    chk["正向_不同tag计划不同"] = bool(cross > 1e-6)

    chk["命中_同tag重复调用逐位一致"] = bool(
        np.array_equal(g_fc, plan(tag_fc, price_real)))

    _PLAN0_CACHE4.clear()
    a = plan("oracle|main|(6, 12, 18)", price_real)
    b = plan("oracle|main|(6, 12, 18)", price_flat)       # 同 tag，价格不同
    chk["负向对照_同tag不同价命中旧计划"] = bool(np.array_equal(a, b))

    _PLAN0_CACHE4.clear()
    g_fx2 = plan(tag_fx, price_flat)
    g_fc2 = plan(tag_fc, price_real)
    chk["顺序无关_交换调用顺序结果不变"] = bool(
        np.array_equal(g_fx2, g_fx) and np.array_equal(g_fc2, g_fc))

    chk["缓存键含tag"] = bool(
        len(_PLAN0_CACHE4) == 0 or
        all(k[0] in {tag_fc, tag_fx, "oracle|main|(6, 12, 18)"}
            for k in _PLAN0_CACHE4))

    _PLAN0_CACHE4.clear()
    bools = [v for v in chk.values() if isinstance(v, bool)]
    chk["结论"] = "通过" if all(bools) else "不通过"

    if verbose:
        print("-" * 74)
        print("D5 缓存隔离自检（p4_microgrid.verify_cache_isolation）")
        print("-" * 74)
...
def run_strategy_comparison(S: dict, cal42: CalConfig, cal43: CalConfig3,
                            ...
                            warmup42: RiskParams | None = None,
                            warmup43: RiskParams | None = None,
                            core: CorePreset | None = None,
                            independent_warmup: bool = True) -> pd.DataFrame:
    """框架 6.1 节的三种策略对照：同一条实际价格路径，各自独立回放。

    ΔJ = J(固定电价参考) − J(波动电价预测)，即"适应波动电价值多少钱"。
    绝不用"旧策略在固定价下的账单"减"新策略在实际价下的账单"。

    标定口径（D3）：论文 §5.4/附录表明三种策略\textbf{各自独立标定}参数，
    用的是\textbf{各自信息集下的历史}（fixed 用附件 1 分时价、forecast 用历史
    预测价、oracle 用当天实际价）。因此本函数在 independent_warmup=True 时对
    每个 mode 各调一次 select_warmup42/43；只有 independent_warmup=False 才退回
    旧行为（三个 mode 共用同一组 forecast 预热参数）——旧行为与论文不符，仅用于
    复现历史数字，不作文档口径。
    """
    rows = []
    indep = bool(independent_warmup and core is not None and core.warmup_grid()
                 and (warmup42 is not None or warmup43 is not None))
    if indep:
        # 每个价格模式各自选一组 (α,ρ,λ)：信息集不同 → 标定结果可以不同
        w42 = {m: select_warmup42(core, S["book"], m, S["LOAD"], S["PV"],
                                  S["eps"], S["fo"], weighted)[0]
               for m in PRICE_MODES}
        w43 = {m: select_warmup43(core, S["book"], m, S["LOAD"], S["PV"], S["fp"],
                                  S["eps3"], S["fo"], "main", weighted)[0]
               for m in PRICE_MODES}
    else:
        w42 = {m: warmup42 for m in PRICE_MODES}
        w43 = {m: warmup43 for m in PRICE_MODES}
    ...
    for m in PRICE_MODES:
        rows.append({"分支": "4-2", "策略": MODE_LABEL[m], "模式": m,
                     "1月标定": (w42[m].label() if w42[m] else "默认"), **t42[m]})
    ...
    for m in PRICE_MODES:
        ...
        rows.append({"分支": "4-3", "策略": MODE_LABEL[m], "模式": m,
                     "1月标定": (w43[m].label() if w43[m] else "默认"), **t43[m]})
...
def main() -> None:
    ...
    ap.add_argument("--check-cache", action="store_true",
                    help="只跑计划缓存的策略隔离自检（D5），不跑寻优")
    ap.add_argument("--shared-warmup", action="store_true",
                    help="策略对照时三个价格模式共用同一组 1 月预热参数（与论文"
                         "「各自独立标定」不符，仅用于复现历史数字；默认各自标定）")
    ...
    if args.check_cache:
        chk = verify_cache_isolation()
        raise SystemExit(0 if chk["结论"] == "通过" else 1)
    ...
    if args.strategies:
        print("\n" + "=" * 78)
        print("三种价格模式对照（同一实际价格路径，"
              + ("共用预热参数" if args.shared_warmup else "各自独立标定") + "）")
        strat = run_strategy_comparison(S, cal42, cal43, weighted,
                                        warmup42, warmup43, core,
                                        independent_warmup=not args.shared_warmup)
