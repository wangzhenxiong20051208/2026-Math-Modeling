...
退款口径的敏感性另写 p4_tables_refund.tex，由 main.tex 在附录单独 \input。
...
def tab_branches(res: dict) -> str:
    ...
    lines += [
        ...
        r"标定，费用一栏不可相加。两者均按实际电价结算（含初始计划费）；主计费"
        r"口径下不允许下调，故 result4-3 的下调量为零。",
        r"  \end{minipage}",
        r"\end{table}",
        "",
    ]
...
def tab_combo_detail(res: dict) -> str:
    ...
    lines += [
        ...
        r"\textbf{边际价值最低}的一条；表中\textbf{增量为负}的行意味着在已有预报"
        r"之上再订一版\textbf{反而更贵}——该现象及其成因见附录该节说明。",
        r"  \end{minipage}",
        r"\end{table}",
        "",
    ]
...
def tab_refund(res: dict) -> str:
    ...
    退款口径只是一项敏感性——调减部分的原价是否退回，题目没有说明。两种口径
    下模型与执行规则完全相同，只有费用函数不同，因此这张表回答的是"主结论
    对这条歧义有多敏感"，而不是给出第二个答案。
    """
    main, rf = res.get("4-3 主策略"), res.get("4-3 退款口径")
    if not main or not rf:
        return ("% 退款口径未计算（需以 --refund 运行 p4_microgrid.py）\n")
    rows = [
        ("合计费用", "合计费用_元", 2),
        ("其中 计划费", "计划费_元", 2),
        ("其中 调整费", "调整费_元", 2),
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"  \centering",
        r"  \caption{4-3 在主计费口径与退款口径下的对照（两口径各自独立标定并"
        r"回放，模型与执行规则完全相同，只换费用函数）}",
        r"  \label{tab:p4-refund}",
        r"  \footnotesize",
        r"  \begin{tabular}{lrrr}",
        r"    \toprule",
        r"    指标 & 主口径（不退款、只追加） & 退款口径 & 差异 \\",
        r"    \midrule",
    ]
    for name, key, nd in rows:
        a, b = main.get(key), rf.get(key)
    ...
    lines += [
        ...
        r"  \par\vspace{2pt}",
        r"  \begin{minipage}{.92\textwidth}\footnotesize",
        r"    注：退款口径下“少买”不再无条件划算——调减会退回原价，可能诱使"
        r"计划量虚高、把电量买在未必便宜的时段。因此两种口径的\textbf{标定结果"
        r"与最终计划都不同}，表中的差异同时包含了参数差异与规则差异，不能读作"
        r"同一个解的两种计价。这里关心的是\textbf{4-3 相对 4-2 的方向是否依然"
        r"成立}：只要结论不变，主结论就不依赖这条题面歧义。",
        r"  \end{minipage}",
        r"\end{table}",
        "",
    ]
