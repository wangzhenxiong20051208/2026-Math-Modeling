...
SUP = ROOT / "08_算法修正" / "输出"
OUT_TEX = ROOT / "08_算法修正" / "输出" / "论文表格" / "p3_tables.tex"
...
OUT_TEX_TAIL = ROOT / "08_算法修正" / "输出" / "论文表格" / "p3_tables_appendix.tex"
...
def main() -> None:
    ...
    if "--split" in sys.argv:
        ...
        tail = HEADER + [tab_combo(res), tab_skill(res), tab_check(res)]
        OUT_TEX.write_text("\n".join(body), encoding="utf-8")
        OUT_TEX_TAIL.write_text("\n".join(tail), encoding="utf-8")
        print(f"写入 {OUT_TEX}（正文：表 1/2/3 格式结果）")
        print(f"写入 {OUT_TEX_TAIL}（附录：组合比较 + 预报精度 + 七项校验）")
        return
    parts = HEADER + [
        tab_skill(res),
        tab_buy(res),
