...
def validate_day(d, price, load_kw, pv_kw, out):
    ...
    # 6 购电量非负（下调幅度已由 up/down 定义约束，无需另行判据）
...
def main():
    ...
    # specified dates：把 SPEC 的日期字符串映射到 dates 数组下标
    import datetime
    spec_idx={}
    for s in SPEC:
    ...
    for s,i in spec_idx.items():
        ...
        while t<N:
            if r[t]>1e-6:
                ...
                s1=t
                ev.append({"start":fmt_slot(s0).split("-")[0],"end":fmt_slot(s1-1).split("-")[1],"kwh":float(r[s0:s1].sum()),"cost":float(np.sum(5*price[s0:s1]*r[s0:s1]))})
            else: t+=1
...
def export_result3(price,dates,res):
    # 每次强制从官方模板重新拷贝：确保表头标签、列口径、残留行都来自模板，
    # 不沿用上一次输出文件里的历史内容（否则旧表头/脏数据会被带入交付件）。
    import shutil
    assert TEMPLATE3.exists(), f"官方模板不存在: {TEMPLATE3}"
    shutil.copyfile(TEMPLATE3, OUT_XLSX)
    ...
    for sheet in ["计划购电量","调整购电量"]:
        ...
        for r,d in enumerate(range(31,365), start=2):
            ...
            # 全天购电费：计划表填计划费 Σp·gP；调整表填计划调整总费 Σ(p·gP+1.5p·up−0.5p·down)。
            # 紧急购电费不在本列，单独在论文正文与 p3_spec.json 中列出。
            up=(res[d]["gA"]-res[d]["gP"]).clip(min=0)
            down=(res[d]["gP"]-res[d]["gA"]).clip(min=0)
            if sheet=="计划购电量":
                cost_pa=float(np.sum(price*res[d]["gP"]))
            else:
                cost_pa=float(np.sum(price*res[d]["gP"]+1.5*price*up-0.5*price*down))
